...
test("timelines", async t => {
  ...
  const log2 = await request(app).post('/login').send({
    "password": "testyPass",
    "username": "testyTest2"
  }).set('Accept', 'application/json');
  ...
  const msg2 = await request(app).post('/add_message').send({
    "text": "someTestyText2",
    "flagged": false
  }).set('Accept', 'application/json');
  ...
  const rvpublic = await request(app).get('/public').send().set('Accept', 'application/json');
  //test that we got both messages
  t.is(rvpublic.body.result.filter(el => el.text == "someTestyText1").length, 1);
  t.is(rvpublic.body.result.filter(el => el.text == "someTestyText2").length, 1);

  // timeline of 2 shows just his messages
  let rv = await request(app).get('/').send().set('Accept', 'application/json');
  ...
  // 2 follows 1
  rv = await request(app).get('/testyTest/follow').send();
  rv = await request(app).get('/').send().set('Accept', 'application/json');
  ...
  rv = await request(app).get('/testyTest2').send();
  ...
  rv = await request(app).get('/testyTest').send();
  ...
  rv = await request(app).get('/testyTest/unfollow').send();
  rv = await request(app).get('/').send().set('Accept', 'application/json');
  t.is(rv.body.result.filter(el => el.text == "someTestyText1").length, 0);
  t.is(rv.body.result.filter(el => el.text == "someTestyText2").length, 1);
